$(function () {
	...
	// 	uploader : uploadUrl,	//PHP处理脚本地址【var uploadUrl = '{:U("Common/uploadFace")}';】
	// 	width : 120,	//上传按钮宽度
	// 	height : 30,	//上传按钮高度
	// 	buttonImage : PUBLIC + '/Uploadify/browse-btn.png',	//上传按钮背景图地址
	// 	fileTypeDesc : 'Image File',	//选择文件提示文字
	// 	fileTypeExts : '*.jpeg; *.jpg; *.png; *.gif',	//允许选择的文件类型
	// 	//由于是上传到构造函数中，有登录session验证所以要加入TP自带的读取,[var sid = '{:session_id()}';]
     //    formData : {'session_id' : sid},
	// 	//上传成功后的回调函数,传入file文件名，data返回的数据,第三个
	// 	onUploadSuccess : function (file, data, response) {
	// 		eval('var data = ' + data);
	// 		if (data.status) {
	// 			$('#face-img').attr('src', ROOT + '/Uploads/face/' + data.savepath.max);
	// 			$('input[name=face180]').val(data.savepath.max);
	// 			$('input[name=face80]').val(data.savepath.medium);
	// 			$('input[name=face50]').val(data.savepath.mini);
	// 		} else {
	// 			alert(data.msg);
	// 		}
	// 	}
